...
from PyQt5.QtWidgets import QFileDialog, QPushButton, QCheckBox, QLineEdit, QLabel, QComboBox, QListWidget, QProgressBar, QTabWidget, QWidget, QApplication
...
def browse_path(entry_path, file_types=[("Excel Files", "*.xlsx")], file=True):
    if file:
        filter_string = ";;".join([f"{desc} ({ext})" for file_type in file_types for desc, ext in file_type])
        path, _ = QFileDialog.getOpenFileName(filter=filter_string)
    else:
    ...
    if filepath != "" and os.path.exists(filepath):
        ...
        def on_labels_clicked():
            ...
            if y.label != x.label and y.label in labels and x.label in labels:
                status_label.config(text="QLabels are set")
                def set_test_size():
                    try:
                        ...
                        if test_percentage > 0 and test_percentage < 100:
                            ...
                            for i, m in enumerate(selected_models):
                                model_button = QPushButton(analysis_tab, text=m.name, command=lambda m_name=m.name:open_graph_image(m_name))
                            ...
                            new_analysis_button = QPushButton(analysis_tab, text="Train New Models", command=lambda: (extras.append(new_analysis_button), reset()))
                            new_analysis_button.grid(row=11, column=0, padx=10, pady=10, sticky="ew")
                    except ValueError:
                        status_label.config(text="Please enter an integer")
                        
                set_test_percentage = QPushButton(analysis_tab, text="Set Test % and Download Models", command=set_test_size)
                set_test_percentage.grid(row=9, column=0, columnspan=3, padx=5, pady=5, sticky="ew")
                test_selection = QLabel(analysis_tab, text="Enter test percentage (20% recommended)")
                test_percentage_input= QLineEdit(analysis_tab, width=10)
        ...
        selection_label_x = QLabel(analysis_tab, text="Independent Variable (X-Axis): ")
        x_selection_str = QLineEdit(analysis_tab)
        x_selection_str.set("Select")
        x_dropdown = QComboBox(analysis_tab, x_selection_str, *labels)
        selection_label_y = QLabel(analysis_tab, text="Dependent Variable (Y-Axis): ")
        y_selection_str = QLineEdit(analysis_tab)
        y_selection_str.set("Select")
        y_dropdown = QComboBox(analysis_tab, y_selection_str, *labels)
        set_labels_button = QPushButton(analysis_tab, text="Set QLabels", command=on_labels_clicked)
    ...
    if os.path.exists(path):
        ...
        def predict_button_click():
            try:
                ...
                download_prediction = QPushButton(prediction_tab, text=f"Download predicition for intensity {round(x_val, 2)}", command=lambda: (download_predictions(x_val,predictions, path), status_label.config(text="Downloaded")))
                download_prediction.grid(row=6, column=0, padx=5, pady=5, sticky="ew")
            except ValueError:
                status_label.config(text="Please enter a number")
                return
            
        predict_button = QPushButton(prediction_tab, text="Predict", command=predict_button_click)
        predict_image_entry = QLineEdit(prediction_tab, width=50)
        predict_image_browse = QPushButton(prediction_tab, text="Browse", command=lambda:browse_path(predict_image_entry, [("JPG", "*.jpg"),("PNG", "*.png"),("JPEG", "*.jpeg")]))
...
def perform_image_analysis():
    ...
    if os.path.exists(folder_path):
        ...
        show_image_label = QLabel(image_analysis_tab)
        intensity_label = QLabel(image_analysis_tab, text="")
        progress_bar = QProgressBar(image_analysis_tab, orient="horizontal", length=800, mode="determinate")
        progress_status_bar = QLabel(image_analysis_tab, text="")
...
app_tab_book = QTabWidget(app)
app_tab_book.grid(row=0, column=0, sticky='nsew')


image_analysis_tab = QWidget(app_tab_book)
image_analysis_text = "Image Analysis"
app_tab_book.add(image_analysis_tab, text=image_analysis_text)

analysis_tab = QWidget(app_tab_book)
analysis_tab_text = "Data Analysis"
app_tab_book.add(analysis_tab, text=analysis_tab_text)

prediction_tab = QWidget(app_tab_book)
predict_tab_text = "Predict"
app_tab_book.add(prediction_tab, text=predict_tab_text)

app_tab_book.bind("<<QTabWidgetTabChanged>>", on_tab_selected)

#Analysis Tab elements
img = Image.open("mmne.jpg")#"media/img/mmne.jpg")
img = img.resize((200, (200*img.height//img.width) ))
img = ImageTk.PhotoImage(img)
logo_label = QLabel(analysis_tab, image=img)
analysis_file_path = QLineEdit(analysis_tab, width=50)
analysis_browse_button = QPushButton(analysis_tab, text="Browse", command=lambda: browse_path(analysis_file_path))
listbox_models = QListWidget(analysis_tab, selectmode="multiple", height=len(ML_Model.models))
for model in ML_Model.models:
    listbox_models.insert('end', model.name)
select_all_var = QCheckBox()
select_all_checkbox = QCheckBox(analysis_tab, text="Select All", variable=select_all_var, command=select_all_models)
start_processing_button = QPushButton(analysis_tab, text="Process File", command=process_file, state="normal")
...
predict_path = QLineEdit(prediction_tab, width=50)
browse_predict_path = QPushButton(prediction_tab, text="Browse", command=lambda:browse_path(predict_path, [("Excel Files", "*.xlsx"), ("Pickle", "*pkl")]))
load_model_button = QPushButton(prediction_tab, text="Load Model(s)", command=load_model)
logo_label = QLabel(prediction_tab, image=img)
prediction_label = QLabel(prediction_tab,text="")
...
image_folder_path = QLineEdit(image_analysis_tab, width=50)
browse_folder_path = QPushButton(image_analysis_tab, text="Browse", command=lambda:browse_path(image_folder_path, file=False))
analyse_image_button = QPushButton(image_analysis_tab, text="Perform Analysis", command=lambda: (analyse_image_button.config(state="disabled"), perform_image_analysis(), analyse_image_button.config(state="normal")), background="blue")
logo_label = QLabel(image_analysis_tab, image=img)
image_analysis_label = QLabel(image_analysis_tab, text="")
